...
public class ReportsController {
	...
	//list used for populating the table in pop-up 
	...
	//listener class for login out in Reports tab
	...
	//listener class for closing the pop-up in Reports tab
	class okButtonListener implements ActionListener{
		@Override
		public void actionPerformed(ActionEvent e) {

			try{
				//dispose of the pop-up dialog
	...
	class CheckSelectedListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			calculateItems(); //calculate what should be put in the table for pop-up
			if (index>-1)   //if a row is selected in the table in report panel
			{
				//set table model for pop-up with the two  lists								
				getView().getTabsPane().getReportPanel().getPopup().setTableModel(itemsSold,listq);
				//set the total text-field in pop-up
	...
	//Calculate what items should be placed in the pop-up table
	...
	{//getting the index of the selected row in the table of sales
		...
		if (index>-1)   {
			...
			listq=tempList2.get(1);      //get the quantity                                   												 
